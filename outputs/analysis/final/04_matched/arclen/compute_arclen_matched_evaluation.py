...
FINAL_DIR = Path(__file__).parent.parent.parent  # outputs/analysis/final/
MATCHED_TARGETS_DIR = FINAL_DIR / "04_matched" / "targets"
MATCHED_EVAL_DIR = FINAL_DIR / "04_matched" / "arclen"
...
def compute_pooled_arclen_bins(languages: List[str], split: str = 'test', K: int = 5) -> Tuple[ArcBins, Dict[str, Any]]:
    ...
    for v in pooled:
        ...
        for i in range(len(counts)):
            ...
            if (i < len(counts)-1 and v >= lo and v < hi):
                counts[i] += 1
                break
            elif (i == len(counts)-1 and v >= lo and v <= hi):
                counts[i] += 1
                break
...
def save_target_specification(target_meta: Dict[str, Any], output_file: Path, split: str):
    target_spec = {
        ...
        'created_by': 'compute_arclen_matched_evaluation.py',
...
def main():
    ...
    output_file = MATCHED_EVAL_DIR / f"matched_eval_arclen_per_layer_{args.split}.csv"
    results_df.to_csv(output_file, index=False)
    print(f"  ✓ Results: {output_file} ({len(results_df)} rows)")

    # Canonical copy for test split (align with Stage 7 convention)
    if args.split == 'test':
        canonical_file = MATCHED_EVAL_DIR / "matched_eval_arclen_per_layer.csv"
