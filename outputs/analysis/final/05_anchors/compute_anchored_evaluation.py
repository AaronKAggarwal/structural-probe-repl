...
SENTENCE_STATS_DIR = FINAL_DIR / "01_covariates" / "sentence_stats"
TABLES_DIR = FINAL_DIR / "03_tables"
ANCHORS_DIR = FINAL_DIR / "05_anchors"
...
def main():
    ...
    master_file = TABLES_DIR / "analysis_table_per_layer.csv"
    ...
    anchors_json = {
        ...
        'created_by': 'compute_anchored_evaluation.py'
    }
    with open(ANCHORS_DIR / 'anchors_used.json', 'w') as f:
    ...
    # Save outputs
    out_len = ANCHORS_DIR / f"anchored_eval_len_per_layer_{args.split}.csv"
    out_arc = ANCHORS_DIR / f"anchored_eval_arclen_per_layer_{args.split}.csv"
    ...
    print(f"Wrote: {out_len} ({len(df_len)} rows)")
    print(f"Wrote: {out_arc} ({len(df_arc)} rows)")

    if args.split == 'test':
        df_len.to_csv(ANCHORS_DIR / "anchored_eval_len_per_layer.csv", index=False)
        df_arc.to_csv(ANCHORS_DIR / "anchored_eval_arclen_per_layer.csv", index=False)
    ...
    (ANCHORS_DIR / f"anchored_eval_qc_len_{args.split}.json").write_text(json.dumps(qc_len, indent=2))
    (ANCHORS_DIR / f"anchored_eval_qc_arclen_{args.split}.json").write_text(json.dumps(qc_arc, indent=2))
    print(f"\nQC summaries written to {ANCHORS_DIR}/")
