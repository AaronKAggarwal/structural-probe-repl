...
CONLLU_FILE_REL_PATH = (
    "src/legacy/structural_probe/example/data/en_ewt-ud-sample/en_ewt-ud-dev.conllu"
)
...
data_files_exist = (PROJECT_ROOT / CONLLU_FILE_REL_PATH).exists() and (
    PROJECT_ROOT / HDF5_FILE_REL_PATH
).exists()
...
def test_detailed_outputs_generation(tmp_path: Path):
    ...
    try:
        ...
        assert (
            output_dir_for_assertions / f"train_detailed_metrics_epoch{num_epochs}.json"
        ).exists(), (
            f"train_detailed_metrics_epoch{num_epochs}.json not found in {output_dir_for_assertions}."
        )
        assert (
            output_dir_for_assertions / f"dev_detailed_metrics_epoch{num_epochs}.json"
        ).exists(), (
        ...
        expected_dev_metric_plot_filename = (
            f"dev_{monitor_metric_for_test}_vs_epoch.png"
        )
        assert (
            output_dir_for_assertions / expected_dev_metric_plot_filename
        ).exists(), (
        ...
        assert checkpoints_dir.is_dir(), (
            f"Checkpoints directory not found in {output_dir_for_assertions}."
        )

        # Check for epoch 1 checkpoint (since save_every_epoch_checkpoint=true)
        epoch_files = list(
            checkpoints_dir.glob(
                f"{probe_type}_probe_rank{probe_rank}_epoch{num_epochs}_metric*.pt"
            )
        )
        ...
        pytest.fail(
            f"Smoke test (detailed_outputs) training script failed with exit code {e.returncode}.\n"
        ...
        pytest.fail(
            f"Smoke test (detailed_outputs) encountered an unexpected error: {e}\n{traceback.format_exc()}"
        )
