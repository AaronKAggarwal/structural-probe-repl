    ...
    try:
        ...
        for line in process.stdout.splitlines():
            if "Output directory for this run (from HydraConfig):" in line:
                ...
                match = re.search(
                    r"Output directory for this run \(from HydraConfig\): (.*)", line
                )
                if match:
        ...
        assert run_output_dir is not None, (
            "Could not parse Hydra output directory from script stdout."
        )
        assert run_output_dir.is_dir(), (
            f"Hydra output directory {run_output_dir} was not created or is not a directory."
        )
        ...
        epoch1_checkpoints = list(
            checkpoint_dir.glob(
                f"{probe_type_smoke}_probe_rank{probe_rank_smoke}_epoch1_metric*.pt"
            )
        )
        ...
        best_checkpoint = (
            checkpoint_dir / f"{probe_type_smoke}_probe_rank{probe_rank_smoke}_best.pt"
        )
        ...
        pytest.fail(
            f"Smoke test training script failed with exit code {e.returncode}.\n"
        ...
        pytest.fail(
            f"Smoke test encountered an unexpected error: {e}\n{traceback.format_exc()}"
        )
    finally:
