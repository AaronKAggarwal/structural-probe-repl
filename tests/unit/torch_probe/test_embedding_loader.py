...
from src.torch_probe.utils.embedding_loader import load_elmo_embeddings_for_sentence # Adjust import
...
def test_load_valid_sentence_and_layer(dummy_hdf5_file_fixture):
    with h5py.File(dummy_hdf5_file_fixture, 'r') as hf:
        embeddings = load_elmo_embeddings_for_sentence(hf, "0", 2) # Layer 2 (3rd layer)
...
def test_load_missing_sentence_key(dummy_hdf5_file_fixture):
    with h5py.File(dummy_hdf5_file_fixture, 'r') as hf:
        embeddings = load_elmo_embeddings_for_sentence(hf, "99", 0) # Key "99" doesn't exist
        assert embeddings is None

def test_load_invalid_layer_index(dummy_hdf5_file_fixture):
    with h5py.File(dummy_hdf5_file_fixture, 'r') as hf:
        embeddings = load_elmo_embeddings_for_sentence(hf, "0", 3) # Layer 3 doesn't exist (0,1,2)
        assert embeddings is None
        embeddings_neg = load_elmo_embeddings_for_sentence(hf, "0", -1) # Negative layer
        assert embeddings_neg is None

def test_load_malformed_ndim_data(dummy_hdf5_file_fixture):
    with h5py.File(dummy_hdf5_file_fixture, 'r') as hf:
        embeddings = load_elmo_embeddings_for_sentence(hf, "2", 0) # Key "2" has 2D data
        assert embeddings is None # Should fail ndim check

def test_load_single_layer_stack(dummy_hdf5_file_fixture): # e.g. if ELMo only had 1 output layer
    with h5py.File(dummy_hdf5_file_fixture, 'r') as hf:
        embeddings = load_elmo_embeddings_for_sentence(hf, "3", 0) # Key "3" has shape (1, 8, 1024)
        assert embeddings is not None
