...
from src.torch_probe.train_utils import (  # Adjust import
    EarlyStopper,
    get_optimizer,
    load_checkpoint,
    save_checkpoint,
...
def test_early_stopper_max_mode():
    ...
    assert stopper(
        0.65
    )  # No improvement (not > 0.7 + 0.1), counter = 1 -> early_stop = True
    assert stopper.early_stop
...
def test_early_stopper_no_improvement_stops():
    ...
    def test_save_and_load_checkpoint(tmp_path: Path):
        ...
        save_checkpoint(
            model, optimizer, epoch, best_metric, checkpoint_dir, "test_probe"
        )
        ...
        assert torch.allclose(
            new_model.linear.weight, original_weight.to(expected_device)
        )
