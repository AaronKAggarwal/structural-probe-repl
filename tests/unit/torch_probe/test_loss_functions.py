...
import pytest
import torch
from typing import Optional # Ensure this is imported if not already at the top
from src.torch_probe.loss_functions import distance_l1_loss, depth_l1_loss # Adjust import if needed
...
def test_depth_l1_loss_simple():
    ...
    gold_depths = torch.tensor([[1.0, 5.0, 8.0], [2.0, 3.0, -1.0]])   # Gold NON-SQUARED, -1 is padding
    ...
    expected_loss = ( (0.0 + 1.0 + 1.0)/3.0 + (1.0 + 1.0)/2.0 ) / 2.0 # = (2/3 + 1)/2 = 5/6
...
def test_depth_l1_loss_no_valid_tokens():
    ...
    gold_depths = torch.tensor([[-1.0, -1.0]]) # All values are padding markers if lengths makes them so
...
def test_distance_l1_loss_simple():
    ...
    pred_sq_distances = torch.tensor([[[0.0, 1.0, 4.0],
                                     [1.0, 0.0, 1.0],
                                     [4.0, 1.0, 0.0]]]) # Predicted SQUARED L2
    gold_distances = torch.tensor([[[0.0, 2.0, 3.0],
                                    [2.0, 0.0, 2.0],
                                    [3.0, 2.0, 0.0]]]) # Gold NON-SQUARED
...
def test_distance_l1_loss_with_padding():
    ...
    pred_sq_distances = torch.tensor([[[10.0, 1.0, 99.0],  # 99 is padding prediction
                                     [1.0, 20.0, 88.0],   # Values for i=j can be anything, but sum includes them
                                     [99.0, 88.0, 30.0]]])
    gold_distances    = torch.tensor([[[0.0, 2.0, -1.0],   # -1 is padding gold
                                     [2.0, 0.0, -1.0],
                                     [-1.0, -1.0, -1.0]]])
