...
@pytest.fixture
def dummy_conllu_filepath_fixture(
    tmp_path: Path, dummy_conllu_content_fixture: str
) -> str:
...
def test_probe_dataset_initialization_distance(
    dummy_conllu_filepath_fixture, dummy_hdf5_content_fixture
):
...
def test_probe_dataset_initialization_depth(
    dummy_conllu_filepath_fixture, dummy_hdf5_content_fixture
):
    dataset = ProbeDataset(
        conllu_filepath=dummy_conllu_filepath_fixture,
...
def test_probe_dataset_getitem_distance(
    dummy_conllu_filepath_fixture, dummy_hdf5_content_fixture
):
    dataset = ProbeDataset(
        dummy_conllu_filepath_fixture, dummy_hdf5_content_fixture, 0, "distance", 1024
    )
    item0 = dataset[0]
...
def test_probe_dataset_getitem_depth(
    dummy_conllu_filepath_fixture, dummy_hdf5_content_fixture
):
    dataset = ProbeDataset(
        dummy_conllu_filepath_fixture, dummy_hdf5_content_fixture, 1, "depth", 1024
    )
    item1 = dataset[1]
...
def test_probe_dataset_token_count_assertion(
    tmp_path: Path, dummy_conllu_filepath_fixture
):
    ...
    with h5py.File(hdf5_mismatch_path, "w") as hf:
        hf.create_dataset(
            "0", data=np.random.rand(3, 5, 1024).astype(np.float32)
        )  # 5 ELMo tokens
        hf.create_dataset("1", data=np.random.rand(3, 2, 1024).astype(np.float32))

    dataset = ProbeDataset(
        dummy_conllu_filepath_fixture, str(hdf5_mismatch_path), 0, "depth", 1024
    )
    with pytest.raises(
        AssertionError, match="Token count mismatch for sentence key '0'"
    ):
...
def test_collate_probe_batch_depth(
    dummy_conllu_filepath_fixture, dummy_hdf5_content_fixture
):
    dataset = ProbeDataset(
        dummy_conllu_filepath_fixture, dummy_hdf5_content_fixture, 0, "depth", 1024
    )
    ...
    assert collated_batch["embeddings_batch"].shape == (
        2,
        4,
        1024,
    ...
    assert (
        collated_batch["labels_batch"][1, 2].item() == -1.0
    )  # Padded part of shorter sentence
    assert collated_batch["labels_batch"][1, 1].item() != -1.0  # Original part
    dataset.close_hdf5()


def test_collate_probe_batch_distance(
    dummy_conllu_filepath_fixture, dummy_hdf5_content_fixture
):
    dataset = ProbeDataset(
        dummy_conllu_filepath_fixture, dummy_hdf5_content_fixture, 0, "distance", 1024
    ...
    assert collated_batch["labels_batch"].shape == (
        2,
        4,
        4,
    ...
    assert (
        collated_batch["labels_batch"][1, 1, 1].item() != -1.0
    )  # Original part (diagonal is 0)
    dataset.close_hdf5()
