...
from torch_probe.utils.conllu_reader import (  # Use our CoNLL reader
    SentenceData,
    read_conll_file,
)
...
def get_word_token_alignment(
    original_words: List[str], subword_tokens: List[str], tokenizer: PreTrainedTokenizer
) -> List[List[int]]:
    """
    ...
    while original_word_idx < len(original_words) and subword_idx < len(subword_tokens):
        ...
        elif current_reconstructed_word and not subtoken.startswith(
            ("Ġ", " ", "##")
        ):  # Continuation of a word for some tokenizers
        ...
        if current_reconstructed_word.lower() == original_words[
            original_word_idx
        ].lower().replace(" ", ""):  # Very basic check
            alignment.append(list(current_word_subtokens))
            current_word_subtokens = []
            current_reconstructed_word = ""
            original_word_idx += 1
        elif subword_idx == len(subword_tokens) - 1 and original_word_idx < len(
            original_words
        ):  # Last subtoken
...
def align_subword_embeddings(
    ...
    if not alignment:  # No words or alignment failed
        return torch.empty(
            (0, subword_embeddings.size(1)), device=subword_embeddings.device
        )
    ...
    for word_subtoken_indices in alignment:
        if not word_subtoken_indices:  # Should not happen with good alignment
            ...
            log.debug(
                f"Empty subtoken list for a word in alignment. Subword_embeddings shape: {subword_embeddings.shape}"
            )
            # If subword_embeddings could be empty (e.g. for empty input string)
            if subword_embeddings.numel() == 0:
                word_embeddings_list.append(
                    torch.empty((0, 0), device=subword_embeddings.device)
                )  # Special case for completely empty
                continue

            word_embeddings_list.append(
                torch.zeros(
                    subword_embeddings.size(1), device=subword_embeddings.device
                )
            )
            continue
        ...
        valid_indices = [
            idx
            for idx in word_subtoken_indices
            if 0 <= idx < subword_embeddings.size(0)
        ]
        if not valid_indices:
            log.warning(
                f"Invalid subtoken indices for a word: {word_subtoken_indices} "
...
def robust_align_subword_embeddings(
    ...
    """
    if (
        subword_embeddings.numel() == 0
    ):  # Handle empty input string resulting in no embeddings
    ...
    for i in range(num_original_words):
        ...
        if subword_indices_for_word:
            ...
            valid_sub_indices = [
                idx
                for idx in subword_indices_for_word
                if 0 <= idx < subword_embeddings.shape[0]
            ]
            if not valid_sub_indices:
                log.warning(
                    f"Word {i} had no valid subword indices after bounds check. Appending zero vector."
                )
            ...
            log.warning(
                f"Word at index {i} (0-indexed) has no corresponding subwords in word_ids. "
...
@hydra.main(
    config_path="../configs", config_name="config_extract", version_base="1.3"
...
def extract_embeddings_main(cfg: DictConfig) -> None:
    ...
    try:
        ...
        model = AutoModel.from_pretrained(
            cfg.model.hf_model_name, output_hidden_states=True
        )
        model.to(device)
        model.eval()
    except Exception as e:
        log.error(
            f"Failed to load model or tokenizer '{cfg.model.hf_model_name}': {e}",
            exc_info=True,
        )
        raise
    ...
    if (
        isinstance(layers_to_extract_config, str)
        and layers_to_extract_config.lower() == "all"
    ):
        layers_to_extract_indices = list(range(num_model_layers))
        log.info(
            f"Extracting all {num_model_layers} layers (0 to {num_model_layers - 1})."
        )
    elif isinstance(
        layers_to_extract_config, (list, ListConfig)
        ...
        raise ValueError(
            f"Invalid format for layers_to_extract: {layers_to_extract_config}. "
    ...
    for (
        split_name,
        conll_rel_path,
    ) in cfg.input_conll_files.items():  # e.g., train: "path/to/train.conllx"
        if conll_rel_path is None:
            log.info(f"Skipping {split_name} as its CoNLL path is null in config.")
            continue

        conll_abs_path = (
            Path(original_cwd) / conll_rel_path
        )  # Resolve relative to original CWD
        if not conll_abs_path.exists():
            log.error(
                f"Input CoNLL file for split '{split_name}' not found: {conll_abs_path}"
            )
            continue
        ...
        layers_str = (
            "all"
            if len(layers_to_extract_indices) == num_model_layers
            else "_".join(map(str, layers_to_extract_indices))
        )

        # Output path now uses the base_output_path from config, resolved from original_cwd
        base_output_path_for_hdf5 = (
            Path(original_cwd) / cfg.output_hdf5.base_output_path
        )
        base_output_path_for_hdf5.mkdir(
            ...
            for sent_idx, sentence_data in enumerate(
                ...
                for layer_idx in layers_to_extract_indices:
                    if 0 <= layer_idx < len(hidden_states_tuple):
                        ...
                        subword_embeddings_one_layer = (
                            hidden_states_tuple[layer_idx].squeeze(0).cpu()
                        )
                        selected_layer_embeddings_for_sent.append(
                            subword_embeddings_one_layer
                        )
                    else:
                        log.error(
                            f"Layer index {layer_idx} out of range for model {cfg.model.hf_model_name}. "
                ...
                if not selected_layer_embeddings_for_sent:
                    log.warning(
                        f"No valid layers extracted for sentence {sent_idx}. Skipping sentence."
                    )
                    continue

                # Stack layers: (num_extracted_layers, num_subwords, hidden_dim)
                stacked_subword_embeddings = torch.stack(
                    selected_layer_embeddings_for_sent, dim=0
                )
                ...
                for layer_idx_in_stack in range(stacked_subword_embeddings.shape[0]):
                    current_layer_subword_embs = stacked_subword_embeddings[
                        layer_idx_in_stack, :, :
                    ]
                    ...
                    if aligned_layer_word_embs.shape[0] != len(original_words):
                        log.error(
                            f"CRITICAL ALIGNMENT ERROR for sentence {sent_idx} (split {split_name}), layer index {layers_to_extract_indices[layer_idx_in_stack]}: "
                    ...
                    aligned_word_embeddings_all_layers_list.append(
                        aligned_layer_word_embs
                    )

                if (
                    not aligned_word_embeddings_all_layers_list
                ):  # If alignment failed for any layer
                    continue  # Skip to next sentence

                # Stack to: (num_extracted_layers, num_original_words, hidden_dim)
                final_word_embeddings_for_sentence = torch.stack(
                    aligned_word_embeddings_all_layers_list, dim=0
                )
                # Save to HDF5
                ...
                hdf5_file.create_dataset(
                    str(sent_idx), data=final_word_embeddings_for_sentence.numpy()
                )
                total_sentences_processed += 1

            log.info(
                f"Finished processing {split_name}. Saved {total_sentences_processed}/{len(parsed_sentences)} sentences to {hdf5_output_path}."
            )
