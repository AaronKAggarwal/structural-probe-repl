...
Outputs:
- outputs/analysis/sentence_stats/UD_XX/{dev,test}_content_stats.jsonl
"""
...
def compute_tree_height(heads: List[int], upos_tags: List[str]) -> Optional[int]:
    ...
    content_to_orig = {i: orig_idx for i, orig_idx in enumerate(content_indices)}
...
def main():
    ...
    for language_slug in sorted(languages):
        for split in ["dev", "test"]:
            ...
            if stats:
                output_path = REPO_ROOT / "outputs" / "analysis" / "sentence_stats" / language_slug / f"{split}_content_stats.jsonl"
