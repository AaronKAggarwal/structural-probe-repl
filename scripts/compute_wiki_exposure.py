...
def try_wikimedia_dumps(wiki_code: str) -> Optional[Tuple[str, int, str]]:
    ...
    print(f"    ⚠ No 2018 dumps found on Wikimedia")
    return None
...
def try_archive_org(wiki_code: str) -> Optional[Tuple[str, int, str]]:
    ...
    print(f"    ⚠ No archives found on Internet Archive")
    return None
...
def compute_wiki_exposure(language_slug: str) -> Optional[WikiExposure]:
    ...
    print(f"📍 {language_slug} → {wiki_code}wiki")
    ...
    print(f"    → {status}: {size_mb:.1f} MB, WikiSize = {wiki_size_log2_mb:.3f}")
    print()
...
def main():
    ...
    if results:
        ...
        # Save results
        output_file = REPO_ROOT / "outputs" / "analysis" / "pretrain_exposure.csv"
        ...
        print("\n✓ Ready to merge into analysis tables")
    else:
