...
def get_optimizer(
    model_parameters: Iterator[nn.Parameter], cfg_optimizer: Any
) -> optim.Optimizer:
    ...
    logger.info(
        f"Initializing optimizer: {name} with LR: {lr}, WeightDecay: {weight_decay}"
    )
...
class LRSchedulerWithOptimizerReset:
    def __init__(
        ...
        self.optimizer_cfg_template = (
            optimizer_cfg  # Keep the original optimizer config structure
        )
    ...
    def step(
        self, current_metric_value: float, model_params: Iterator[nn.Parameter]
        ...
        if self.best_metric_for_lr_decay is None:  # First call
            ...
            if self.verbose:
                logger.info(
                    f"LR Scheduler: Initialized best metric for LR decay to {self.best_metric_for_lr_decay:.6f}"
                )
            return None
        ...
        if improved:
            if self.verbose:
                logger.info(
                    f"LR Scheduler: Metric improved to {current_metric_value:.6f}. Resetting LR decay patience."
                )
            self.best_metric_for_lr_decay = current_metric_value
            self.epochs_without_improvement = 0
        else:
            ...
            if self.verbose:
                logger.info(
                    f"LR Scheduler: No metric improvement for {self.epochs_without_improvement}/{self.lr_decay_patience} epochs for LR decay. "
            ...
            if self.epochs_without_improvement >= self.lr_decay_patience:
                ...
                if new_lr_candidate >= self.min_lr:
                    ...
                    self.epochs_without_improvement = 0
                    self.best_metric_for_lr_decay = (
                        current_metric_value  # Reset best metric after LR decay
                    )
                    if self.verbose:
                        logger.info(
                            f"LR Scheduler: Decaying LR to {self.current_lr:.7e}. Optimizer will be reset. Decays done: {self.lr_decays_done}."
                        )
                    ...
                    if isinstance(self.optimizer_cfg_template, OmegaConf):
                        new_optimizer_cfg = OmegaConf.to_container(
                            self.optimizer_cfg_template, resolve=True
                        )
                    ...
                    new_optimizer_to_return = get_optimizer(
                        model_params, new_optimizer_cfg
                    )
                else:
                    if self.verbose:
                        logger.info(
                            f"LR Scheduler: Proposed new LR ({new_lr_candidate:.7e}) is below min_lr ({self.min_lr:.7e}). No further decay."
                        )
...
class EarlyStopper:
    ...
    def __call__(self, current_metric_value: float) -> bool:
        if self.best_actual_metric is None:
            ...
            if self.verbose:
                logger.info(
                    f"EarlyStopper: Best metric initialized to {self.best_actual_metric:.6f}"
                )
            return False
        ...
        if improved:
            ...
            if self.verbose:
                logger.info(
                    f"EarlyStopper: Improvement! New best metric: {self.best_actual_metric:.6f}"
                )
            self.counter = 0
        else:
            self.counter += 1
            if self.verbose:
                logger.info(
                    f"EarlyStopper: No significant improvement for {self.counter}/{self.patience} epochs for stopping. "
...
def save_checkpoint(
    ...
    checkpoint_dir: Path,
    filename_prefix: str = "probe",
    is_best: bool = False,
):
    checkpoint_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_path = (
        checkpoint_dir
        / f"{filename_prefix}_epoch{epoch}_metric{current_metric_value:.4f}.pt"
    )
...
def load_checkpoint(
    ...
    start_epoch_to_resume = checkpoint.get(
        "epoch", 0
    )  # This is the epoch that was *completed*
    metric_value_of_checkpoint = checkpoint.get(
        "metric_value",
        float("-inf") if EarlyStopper(mode="max").mode == "max" else float("inf"),
    )

    logger.info(
        f"Checkpoint loaded from {checkpoint_path}. Model from completed epoch {start_epoch_to_resume}. Metric: {metric_value_of_checkpoint:.4f}"
    )
    return (
        start_epoch_to_resume + 1,
        metric_value_of_checkpoint,
    )  # Training should start from next epoch
