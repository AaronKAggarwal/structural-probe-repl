...
class FilteredProbeDataset(ProbeDataset):
    ...
    def __init__(self, 
        ...
        super().__init__(conllu_filepath, hdf5_filepath, embedding_layer_index, 
                         probe_task_type, embedding_dim)
            ...
            for idx in range(len(self.parsed_sentences)):
                ...
                try:
                    ...
                    if num_tokens_elmo != num_tokens_conllu:
                        log.warning(f"Token count mismatch for sentence key '{sentence_key}': "
                                   f"CoNLL-U has {num_tokens_conllu} tokens, "
                                   f"ELMo HDF5 has {num_tokens_elmo} embedding vectors. "
                                   f"Skipping this sentence.")
                        self.skipped_sentences.add(idx)
                except Exception as e:
                    log.warning(f"Error processing sentence {sentence_key}: {e}. Skipping.")
                    self.skipped_sentences.add(idx)
            
            log.info(f"Skipped {len(self.skipped_sentences)} sentences with token count mismatches.")
            
        # Create a valid indices mapping
        self.valid_indices = [i for i in range(len(self.parsed_sentences)) if i not in self.skipped_sentences]
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        if not (0 <= idx < len(self.valid_indices)):
            raise IndexError(f"Index {idx} out of bounds for filtered dataset of length {len(self.valid_indices)}")
