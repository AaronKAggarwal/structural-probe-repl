...
def distance_l1_loss(
    ...
    if not (
        ...
        raise ValueError(
            "Input tensors must be 3D and have matching shapes for distance_l1_loss."
        )
    ...
    for i in range(batch_size):
        ...
        for r_idx in range(l):
            for c_idx in range(r_idx + 1, l):
                abs_diff_sum_sent += torch.abs(
                    pred_sent_dists[r_idx, c_idx] - gold_sent_dists[r_idx, c_idx]
                )
                num_pairs_sent += 1
        ...
        if num_pairs_sent > 0:  # Should be true if l >= 2
            ...
            sent_token_mask = torch.ones(
                (l, l), dtype=torch.bool, device=pred_sent_dists.device
            )
            ...
            sum_abs_diff_sent_full_matrix = torch.sum(
                current_sent_abs_diff
            )  # Sum over all LxL elements

            # Normalize by L^2 as per H&M code
            if (
                l > 0
            ):  # Avoid division by zero for zero-length sentences (though caught by l < 2)
...
def depth_l1_loss(
    ...
    if not (
        ...
        raise ValueError(
            "Input tensors must be 2D and have matching shapes for depth_l1_loss."
        )
    ...
    for i in range(batch_size):
        ...
        if (
            torch.sum(valid_mask_sent).item() == 0
        ):  # All tokens in this sentence were padding
            continue
