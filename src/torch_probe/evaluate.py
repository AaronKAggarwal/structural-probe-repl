...
def calculate_spearmanr_hm_style(
    ...
    for orig_len, scores_with_non_punct_len in raw_scores_and_non_punct_lengths_by_orig_length.items():
        ...
        if scores_to_average:
            avg_score_per_orig_length_group_in_hm_range[orig_len] = np.mean(scores_to_average)
...
def calculate_uuas(
    ...
    for i in range(len(all_predicted_distances)):
        ...
        try:
            ...
            log.warning(f"MST calculation failed for sentence index {i}: {e}. UUAS will be 0 for this sentence.")
    ...
    if all_predictions_np:
        ...
        if probe_type == "distance":
            mean_uuas, per_sentence_uuas = calculate_uuas(
                ...
                all_xpos_tags_list, all_upos_tags_list, punctuation_strategy
            )
...
if __name__ == '__main__':
    ...
    mean_s_dist_hm, _, _, _ = calculate_spearmanr_hm_style(
        ...
        min_len_for_spearman_avg=3, max_len_for_spearman_avg=5
    )
    ...
    mean_uuas_xpos, _ = calculate_uuas(pred_dists_u, gold_heads_u, lengths_u, xpos_u, upos_u, "xpos")
    print(f"UUAS (strategy='xpos', expected 1.0): {mean_uuas_xpos:.4f}")
    mean_uuas_upos, _ = calculate_uuas(pred_dists_u, gold_heads_u, lengths_u, xpos_u, upos_u, "upos")
    ...
    mean_root_acc_xpos, _ = calculate_root_accuracy(pred_depths_r, gold_heads_r, lengths_r, xpos_r, upos_r, "xpos")
    print(f"Root Accuracy (strategy='xpos', expected 1.0): {mean_root_acc_xpos:.4f}")
    mean_root_acc_upos, _ = calculate_root_accuracy(pred_depths_r, gold_heads_r, lengths_r, xpos_r, upos_r, "upos")
